...
object PrepRankingData {
  ...
  def run(spark: SparkSession, params: PrepRankingDataParams): Unit = {
    ...
    require(metadata.contains(Constants.LABELS) && metadata(Constants.LABELS).size == 1,
  ...
  private def readMetadata(fs: FileSystem, path: String): Map[String, Seq[TensorMetadata]] = {
    ...
    JsonUtil.toMap[Seq[TensorMetadata]](metaDataJson)
  }
  ...
  def prepareRanking(
    ...
    val contentFeaturesWithSpVec = contentFeatures.filter(it =>
  ...
  private def updateMetadata(
    ...
    val updateMetadataJson = JsonUtil.toJsonString(updateMetadata)
    val fs = FileSystem.get(spark.sparkContext.hadoopConfiguration)
    IOUtils
      .writeContentToHDFS(
        fs,
